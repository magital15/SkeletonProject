...
__global__
void addMods2D(PolyDevice d_c, PolyDevice d_a, PolyDevice d_b, int* primes)
{
...
void moveToDevice2D(Poly in, PolyDevice d_in)
{
	...
	{
//		d_in.members[i].coeffs = (int*)calloc(len, sizeof(int));
		cudaMalloc(&d_in.members[i].coeffs, len*sizeof(int));
		cudaMemcpy(d_in.members[i].coeffs, in.members[i+1].coeffs, len*sizeof(int), cudaMemcpyHostToDevice);		
	}

//	printf("d_in[0].length is: %i\n", d_in[0].length);
}

void moveFromDevice2D(Poly out, PolyDevice d_out)
{
	int len = d_out.length;

	for (int i = 0; i < NUMPRIMES; i++)
	{
		cudaMemcpy(out.members[i+1].coeffs, d_out.members[i].coeffs, len*sizeof(int), cudaMemcpyDeviceToHost);
		cudaFree(d_out.members[i].coeffs);
	}
...
{
	int* in_array = (int*)calloc(in.length*(NUMPRIMES), sizeof(int));
	int pos = 0;
	in_array[0] = in.length;
	for (int i = 0; i < NUMPRIMES; i++)
	{
		for (int j = 0; j < in.length; j++)
		{
			pos += 1;
			in_array[pos] = in.members[i].coeffs[j];
		}	
	}
	cudaMalloc(&d_in, (NUMPRIMES)*in.length*sizeof(int));
	cudaMemcpy(d_in, in_array, (NUMPRIMES)*in.length*sizeof(int), cudaMemcpyHostToDevice);
...
{
	int* out_length = (int*)calloc(1, sizeof(int));
	cudaMemcpy(out_length, d_out, sizeof(int), cudaMemcpyDeviceToHost);
	out.length = out_length[0];

	int* out_array = (int*)calloc(out.length*(NUMPRIMES+1), sizeof(int));
	cudaMemcpy(out_array, d_out, (NUMPRIMES+1)*out.length*sizeof(int)*sizeof(int), cudaMemcpyDeviceToHost);
	...
	{	
		...
		{
			out.members[i].coeffs[j] = out_array[pos];
		}
	}
	cudaFree(d_out);
}
...
void addPolysTest2(PolyDevice c, PolyDevice a, PolyDevice b, int* primes)
{
	...
	{
		len = a.length;
		b = copyIntoBiggerDevice(b, a.length);
	}
	...
	{
		len = b.length;
		a = copyIntoBiggerDevice(a, b.length);
	}

	
	dim3 threadsPerBlock(16, 16);
	dim3 numBlocks(len / threadsPerBlock.x, NUMPRIMES / threadsPerBlock.y);
	addMods2D<<<numBlocks, threadsPerBlock>>>(c, a, b, primes);	
...
void addPolysTest(Poly a, Poly b, int* primes)
{
	...
	//Allocate memory for device arrays
	cudaMalloc(&d_a, d_out_len*sizeof(int));
	cudaMalloc(&d_b, d_out_len*sizeof(int));

	// Do this for all polys in Polyset
	for (int i = 1; i < NUMPRIMES+1; i++) {
		// Copy input data from host to device
		cudaMemcpy(d_a, a.members[i].coeffs, d_out_len*sizeof(int), 
				   cudaMemcpyHostToDevice);
		cudaMemcpy(d_b, b.members[i].coeffs, d_out_len*sizeof(int), 
				   cudaMemcpyHostToDevice);
  
		// Launch kernel to compute and store modded polynomial values
		addMods<<<(d_out_len + TPB - 1)/TPB, TPB>>>(d_out, d_a, d_b, 
											  primes[i-1], d_out_len);

		// reconstruct answer

		// Copy results from device to host
		cudaMemcpy(c.members[i].coeffs, d_out, d_out_len*sizeof(int), 
				   cudaMemcpyDeviceToHost);
	}
...
{
	// Create a PolyDevice to store data into
	PolyDevice result;

	// Create a reference length
	int origLen = a.length;

	// Give the result the required length
	result.length = len;

	// Copy the original data into the new polydevice
	for (int i = 0; i < NUMPRIMES; i++)
	{
		cudaMalloc(&result.members[i].coeffs, len*sizeof(int));
		cudaMemcpy(result.members[i].coeffs, a.members[i].coeffs, origLen*sizeof(int), cudaMemcpyDeviceToDevice);
	}
