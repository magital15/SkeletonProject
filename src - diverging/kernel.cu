...
__global__
void addModsTest(int* d_out, int* d_a, int* d_b, int* primes, int size)
{
	const int i = blockIdx.x*blockDim.x + threadIdx.x;
	if (i > size) return;
	const float x = d_a[i];
    const float y = d_b[i];
	d_out[i] = getRemainder(x + y, primes[0]);
...
void moveFromDevice(Poly out, int* d_out)
{
	...
	int* out_array = (int*)calloc(out.length*NUMPRIMES, sizeof(int));
	cudaMemcpy(out_array, d_out, NUMPRIMES*out.length*sizeof(int)*sizeof(int), cudaMemcpyDeviceToHost);

	out = copyIntoBigger(out, out.length);
	int pos = 0;
...
void addPolysTest(int* d_a, int* d_b, int* d_c, int* primes)
{
//	int len = d_a[0] <= d_b[0] ? d_a[0] : d_b[0];
	int len = 9;
	// Declare pointers to device arrays
	int *d_out = 0;

	//Allocate memory for device arrays
	cudaMalloc(&d_out, NUMPRIMES*len*sizeof(int) + 1);

	// Launch kernel to compute and store modded polynomial values
	addModsTest<<<(len + TPB - 1)/TPB, TPB>>>(d_out, d_a, d_b, primes, len);

	// Copy results from device to host
	cudaMemcpy(d_c, d_out, NUMPRIMES*len*sizeof(int) + 1, cudaMemcpyDeviceToHost);
  
	// Free the memory allocated for device array
	cudaFree(d_out);
}
